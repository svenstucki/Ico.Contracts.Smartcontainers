...
describe('MiniMeToken test', () => {
  ...
  before(async () => {
    ...
    const tl = testrpc.listen(8546, '127.0.0.1');
    console.log('testrpc:', tl);
  ...
  it('should deploy all the contracts', async () => {
    miniMeToken = await MiniMeToken.new(
      ...
      'MMT',
      true);
    assert.ok(miniMeToken);
    miniMeTokenState = new MiniMeTokenState(miniMeToken, accounts);
  }).timeout(20000);
  ...
  it('Should Create the clone token', async () => {
    b[5] = web3.eth.blockNumber;
    log(`b[5]->  ${b[5]}`);

    miniMeTokenClone = await MiniMeToken.new(
      miniMeToken.address,
      b[5],
      'Clone Token 1',
      18,
    ...
    miniMeTokenCloneState = new MiniMeTokenState(miniMeTokenClone, accounts);
